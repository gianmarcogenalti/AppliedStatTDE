faces<-function(xy=rbind(1:3,5:3,3:5,5:7),which.row,fill=F,nrow,ncol,
                scale=T,byrow=F,main,labels){

spline<-function(a,y,m=200,plot=F){
    n<-length(a)
  h<-diff(a)
  dy<-diff(y)
  sigma<-dy/h
...
xy<-xy[,(h<-rep(1:mm,ceiling(n.char/mm))),drop=F]
if(fill) xy[,-(1:n.c)]<-0
...
face.obj<-list(
    ...
    ,lipsi=rbind(face$lipso["lipsiend",],face$lipsi,
                 invert(face$lipsi[lipsi.refl.ind,,drop=F]),
                 invert(face$lipso["lipsiend",,drop=F]))
    ...
    ,hair=rbind(face$shape["hairend",],face$hair,invert(face$hair[hair.refl.ind,]),
                invert(face$shape["hairend",,drop=F]))
    ,shape=rbind(face$shape,invert(face$shape[shape.refl.ind,]))
)

plot(1,type="n",xlim=c(-105,105)*1.1, axes=F,
