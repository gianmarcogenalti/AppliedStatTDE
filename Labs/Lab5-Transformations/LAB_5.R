...
legend('bottomright',c(expression(lambda[x]),expression(lambda[y]),expression(paste(lambda,'=1'))),col=c('blue','red','grey'),lty=c(1,1,1))
...
#     { m \in R^4 t.c. n * (x.mean-m)' %*% x.invcov %*% (x.mean-m)
#                                                  < cfr.fisher }
